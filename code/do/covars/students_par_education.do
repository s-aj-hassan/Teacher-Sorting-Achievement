...
* Search for parents' education in UDDA 
	...
	foreach par in mom dad { 
		...
		rename pnr `par' 
		bysort `par' (hf_vfra): ge n = _n
		reshape wide hf_vfra hfaudd, i(`par') j(n)
		save $tmp/e_`par', replace 
	}
...
* The variable is categorical and missings are set to = 0 
	...
	use $tmp/students_demographics, clear 
	keep pnr bdate mom dad 
	
	foreach par in mom dad { 

		merge m:1 `par' using $tmp/e_`par', keep(1 3) nogen 

		reshape long hf_vfra hfaudd, i(pnr mom dad bdate) j(j)
...
* Save data 
	...
	forval j = 1/15 { 
		...
		label var mom_educ`j' "Mother's education at child age `j'"
		label var dad_educ`j' "Father's education at child age `j'"
		
	}
	
	save $tmp/students_par_education, replace 
